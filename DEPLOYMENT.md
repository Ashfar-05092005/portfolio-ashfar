...
**Option A: Gmail (requires App Password)**
   ...
   - `CONTACT_EMAIL=yourgmail@gmail.com` (recipient email)
...
**Option B: Mailtrap (easiest for testing)**
   ...
   - `CONTACT_EMAIL=any@email.com` (Mailtrap sends all mail to your inbox)
     - `FRONTEND_URL=https://ashfarm.onrender.com` (frontend URL without trailing slash)
...
3. The frontend should POST JSON to `/api/contact`.
...
- **SMTP troubleshooting**: Check Render logs for startup lines that confirm the configured frontend URL and contact email.
